...
:mSoundRef(0), mModule(0)
{
    ...
    gCameraService = this;
    //jmq.for switch test, load camera sound here, and only do once.jmq.will load sound failed, why?
  //   loadSound();
}
...
CameraService::~CameraService() {
    ...
    //jmq.for switch test, release camera sound here
    //releaseSound();
...
//#define USE_NATIVE_SOUNDPOOL
#ifdef USE_NATIVE_SOUNDPOOL
SoundPool *gAp = new SoundPool(2, 3/*AUDIO_STREAM_MUSIC*/, 0);
uint16_t            mSampleID[NUM_SOUNDS]={-1,-1};
void CameraService::loadSound() {
    Mutex::Autolock lock(mSoundLock);
    LOG1("CameraService::loadSound ref=%d", mSoundRef);
    if (mSoundRef++) return;
    mSampleID[SOUND_SHUTTER] = gAp->load("/system/media/audio/ui/camera_click.ogg",1);
    if(mSampleID[SOUND_SHUTTER] < = 0)
	LOGE("LOAD sound 1 error\n");
    mSampleID[SOUND_RECORDING] = gAp->load("/system/media/audio/ui/VideoRecord.ogg",1);
    if(mSampleID[SOUND_RECORDING] < = 0)
	LOGE("LOAD sound 2 error\n")
}

void CameraService::releaseSound() {
    Mutex::Autolock lock(mSoundLock);
    LOG1("CameraService::releaseSound ref=%d", mSoundRef);
   if (--mSoundRef) return;//jmq.temp
    if(gAp != NULL)
    {
        delete gAp;
	 gAp = NULL;
...
void CameraService::playSound(sound_kind kind) {
    ...
    Mutex::Autolock lock(mSoundLock);
    if(gAp)
        gAp->play(mSampleID[kind], 1,1, 0, 0, 1);
}
#else
void CameraService::loadSound() {
    Mutex::Autolock lock(mSoundLock);
...
void CameraService::releaseSound() {
    Mutex::Autolock lock(mSoundLock);
    LOG1("CameraService::releaseSound ref=%d", mSoundRef);
  //  if (--mSoundRef) return;//jmq.not care ref
...
CameraService::Client::Client(const sp<CameraService>& cameraService,
    ...
    cameraService->loadSound();//jmq.for switch test, not load every time
...
CameraService::Client::~Client() {
    ...
    disconnect();
    mCameraService->releaseSound();//jmq.for switch test, not release every time
