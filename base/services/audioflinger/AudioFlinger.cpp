...
#ifdef SLSI_ULP_AUDIO
void AudioFlinger::makeVizBuffers(uint8_t *dst, const char *src, int size)
{
    int len = size * 4;
    ...
    short monoData[size];
    int8_t data[size];
    short *rpBuff = (short *)src;

    /* change to mono from stereo */
    while (len > 0) {
        monoData[offset++] = ((int(rpBuff[0]) + int(rpBuff[1])) >> 1);
        rpBuff += 2;
        len -= frameSize;
        if (offset >= size) {
            offset = 0;
    ...
    /* change to signed 8bit from signed 16bit */
    for (offset = 0; offset < size; offset++) {
        data[offset] = monoData[offset] >> 8;
    }

    /* change to unsigned 8bit from signed 8bit */
    for (offset = 0; offset < size; offset++) {
        dst[offset] = data[offset] + 128;
    }
}

bool AudioFlinger::getWaveForm(uint8_t *waveform, uint32_t size)
{
    char *pcmData;

    pcmData = (char *)SRP_Ctrl_Get_Pcm();
    if (pcmData == NULL)
        return false;

    makeVizBuffers(waveform, pcmData, size);
