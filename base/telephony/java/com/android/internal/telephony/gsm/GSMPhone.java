...
import android.net.LocalServerSocket; //chy add
import android.net.LocalSocket; //chy add
...
public class GSMPhone extends PhoneBase {
	...
	Thread testPortThread; //chy add
	private static LocalServerSocket testSocket;
    //ServerSocket debugSocket; //chy commend
	private static LocalServerSocket debugSocket;
    ...
    GSMPhone (Context context, CommandsInterface ci, PhoneNotifier notifier, boolean unitTestMode) {
        ...
        //if (false) {
        //chy open it for receive the reset modem message. 20120329
        if (true) {
            try {
                            ...
                            public void run() {
                                for(;;) {
                                    try {
                                        ...
                                        Log.i(LOG_TAG, "New connection; Msg:cp_reset, resetting radio");
											
										int resetType = 0;
										Log.i(LOG_TAG, "Reset type = "+resetType);
										Message msg = obtainMessage(resetType);
										Log.i(LOG_TAG, "===Reset radio ====");
										mCM.resetRadio(msg);
        ...
        if (true) {
            try {
				//debugSocket = new LocalServerSocket("GSM2VOLD_SOCKET");
                testSocket = new LocalServerSocket("com.android.internal.telephony.test");
                            ...
                            public void run() {
                                for(;;) {
                                    try {
                                        ...
                                        sock = testSocket.accept();
                                        Log.i(LOG_TAG, "New connection; Msg:cp_reset_in_l3, resetting radio");
										
										int resetType = 1;
										Log.i(LOG_TAG, "Reset type = "+resetType);
										Message msg = obtainMessage(resetType);
										Log.i(LOG_TAG, "===Reset radio in AP sleep====");
										mCM.resetRadio(msg);
