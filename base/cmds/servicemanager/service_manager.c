...
//    LOGI("add_service('%s',%p) uid=%d\n", str8(s), ptr, uid);
    ...
    if (si) {
        ...
        si->death.ptr = si;
        si->next = svclist;
		//svclist是一个列表，里面保存了当前注册到ServiceManager中的信息
        svclist = si;
    }
	//当Service退出后，系统通知释放上面malloc出来的资源
...
int main(int argc, char **argv)
{
	...
	//句柄为0,而其余的service的远程接口的句柄都是大于0，这也是ServiceManager成为守护进程的特殊之处
    void *svcmgr = BINDER_SERVICE_MANAGER;//(void *)0

    bs = binder_open(128*1024);//打开binder设备(<4M)

    if (binder_become_context_manager(bs)) {	//成为manager,即binder的守护进程
