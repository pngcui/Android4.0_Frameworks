...
{
    int fd;	//设备文件
    void *mapped;	//设备文件映射到内存空间的起始地址
    unsigned mapsize;	//设备文件映射到内存空间的大小
};
...
//#define BINDER_SET_CONTEXT_MGR _IOW('b',7,int)
    return ioctl(bs->fd, BINDER_SET_CONTEXT_MGR, 0);	//把自己变成manager
}
...
int binder_write(struct binder_state *bs, void *data, unsigned len)
{
	...
	/*
	用户空间程序和Binder驱动程序交互大多数都是通过BINDER_WRITE_READ命令的，
...
void binder_loop(struct binder_state *bs, binder_handler func)
{
	...
	//在binder_write()函数中bwr.write_size = 4;bwr.read_size = 0  
