...
IPCThreadState* IPCThreadState::self()
{
    if (gHaveTLS) {
restart:
	//TLS:Thread Local Storage 每个线程都有一个这样的空间，线程间不共享该空间
        const pthread_key_t k = gTLS;
	//在构造函数中有pthread_setspecific in line644
...
status_t IPCThreadState::waitForResponse(Parcel *reply, status_t *acquireResult)
{
    ...
    while (1) {
		//使用talk...方法把数据发送出去，然后从driver中读到数据并放入mIn中
    ...
    int32_t handle, uint32_t code, const Parcel& data, status_t* statusBuffer)
{
	...
	//mOut是命令的缓冲区，和binder设备并没有什么联系，所以不是在这里和binder通信的
	//真正的通信是在waitForResponse函数中
    mOut.writeInt32(cmd);
