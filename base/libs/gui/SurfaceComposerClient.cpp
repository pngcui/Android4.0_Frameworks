            ...
            uint32_t minLayerZ, uint32_t maxLayerZ)
{
	...
	rtnstatus = s->captureScreen(0, &mHeap,
       ...
       if(rtnstatus != 0 )
       {
            LOGE("Error return from s->captureScreen, mFormat:0x%x mWidth:0x%x mHeight:0x%x",
		mFormat,mWidth,mHeight);
            return rtnstatus;
       }
	//Rotate the data here

	LOGD("updateinternal,mFormat:0x%x mWidth:0x%x mHeight:0x%x",
		mFormat,mWidth,mHeight);
	
	if(mFormat != PIXEL_FORMAT_RGBA_8888)
	{
	       LOGE("The screenshot data format is not PIXEL_FORMAT_RGBA_8888, please check the service end.");
		return rtnstatus;
	}
