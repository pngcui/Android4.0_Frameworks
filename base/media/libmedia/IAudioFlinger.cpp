...
#ifdef SLSI_ULP_AUDIO
    virtual bool getWaveForm(uint8_t *waveform, uint32_t size)
    {
        if(waveform == NULL)
            return BAD_VALUE;
        ...
        status_t status = remote()->transact(GET_RP_DUMP, data, &reply);
        if(status != NO_ERROR) {
            LOGI("getWaveForm Transact fail!");
            return status;
        }
        reply.read(waveform, size);
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        case GET_RP_DUMP: {
            ...
            uint32_t size = 0;
            size = data.readInt32();
            status_t status = getWaveForm(wave, size);
            reply->write(wave, size);
            reply->writeInt32(status);
            return status;
        } break;
#endif
