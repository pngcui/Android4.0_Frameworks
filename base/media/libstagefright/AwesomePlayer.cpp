...
static const char *AUDIO_MIO_LIBRARY_NAME = "libaudiohw.so";
static const char *AUDIO_MIO_FACTORY_NAME = "createAudioMio";
static const char *AUDIO_MIO_DESTROY_NAME = "destroyAudioMio";

/* ULP-class factory function */
typedef AudioPlayerBase * (*AudioMioFactory)(const sp<MediaPlayerBase::AudioSink> &audioSink,
                                         AwesomePlayer *observer);
typedef void (*AudioMioDestroy)(AudioPlayerBase *audioPlayer);

static AudioMioFactory gCreateAudio = NULL;
static AudioMioDestroy gDestroyAudio = NULL;
#endif

/*fix play resume bug (audio seek to 0 sometimes)*/
bool isAudioSeekSkip = false;
...
#ifdef SLSI_ULP_AUDIO
    if (mLibAudioHandle != NULL) {
        dlclose(mLibAudioHandle);
    }
#endif
}
...
#ifdef SLSI_ULP_AUDIO
    if (gDestroyAudio != NULL) {
        gDestroyAudio(mAudioPlayer);
        gDestroyAudio = NULL;
    } else {
        delete mAudioPlayer;
    }
...
#ifdef SLSI_ULP_AUDIO
                if (mIsULPAudio) {
                    mLibAudioHandle = dlopen(AUDIO_MIO_LIBRARY_NAME, RTLD_NOW);
                    if (mLibAudioHandle != NULL) {
                        dlerror();    /* clear error */
                        gCreateAudio = (AudioMioFactory)dlsym(mLibAudioHandle, AUDIO_MIO_FACTORY_NAME);
                        gDestroyAudio = (AudioMioDestroy)dlsym(mLibAudioHandle, AUDIO_MIO_DESTROY_NAME);
                        if (!gCreateAudio || !gDestroyAudio) {
                            ...
                            mAudioPlayer = gCreateAudio(NULL, this);
                            if (mAudioPlayer->initCheck() != OK) {
                                LOGW("ULP-Audio is not created. Create original legacy path");
                                gDestroyAudio(mAudioPlayer);
                                gDestroyAudio = NULL;
                                ...
                                LOGI("ULP audio hardware module loaded");
                            }
                        }
                    } else {
                        LOGW("ULP audio hardware module not found. Create original legacy path");
...
status_t AwesomePlayer::setNativeWindow_l(const sp<ANativeWindow> &native) {
    ...
    if (mLastVideoTimeUs >= 0) {
        ...
        //for CTS testVideoSurfaceResetting
        if (mAudioPlayer != NULL) {
            mSeekTimeUs = mAudioPlayer->getMediaTimeUs();
        } else {
            mSeekTimeUs = mLastVideoTimeUs;
        }
...
#ifdef SLSI_ULP_AUDIO
    } else if (!strcasecmp(mime, MEDIA_MIMETYPE_AUDIO_MPEG) && mUseULPAudio) {
        mAudioSource = mAudioTrack;
        mIsULPAudio = true;
#endif
    } else {
...
void AwesomePlayer::finishSeekIfNecessary(int64_t videoTimeUs) {
    ...
    if (mAudioPlayer != NULL) {
	 //Sometime videoevent delay after play when resume. Return when videoTimeUs =0. yujian@SSCR at 120118
	 if(videoTimeUs == 0 && isAudioSeekSkip == false){
	 	LOGW("==Fixing resume bugs, mSeeking: %d.==", mSeeking);
		isAudioSeekSkip = true;
		return;
	 }		
...
void AwesomePlayer::onCheckAudioStatus() {
    {
        Mutex::Autolock autoLock(mAudioLock);
#ifdef SLSI_ULP_AUDIO
        if (!mAudioStatusEventPending || mAudioPlayer == NULL) {
#else
        if (!mAudioStatusEventPending) {
#endif
...
#ifdef SLSI_ULP_AUDIO
status_t AwesomePlayer::ulpBypass(bool on)
{
    LOGI("ulpBypass(%d)", on);
