...
static void
android_media_MediaPlayer_ulpBypass(JNIEnv *env, jobject thiz, jboolean on)
{
    sp<MediaPlayer> mp = getMediaPlayer(env, thiz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
    ...
    process_media_player_call( env, thiz, mp->ulpBypass(on), NULL, NULL );
}
#endif
// ----------------------------------------------------------------------------
...
#ifdef SLSI_ULP_AUDIO
    {"ulpBypass",           "(Z)V",                             (void *)android_media_MediaPlayer_ulpBypass},
#endif
